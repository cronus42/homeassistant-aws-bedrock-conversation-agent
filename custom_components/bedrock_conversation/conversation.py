"""Conversation entity for AWS Bedrock."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: BedrockConfigEntry,
    async_add_entities: AddEntitiesCallback
) -> bool:
    """Set up AWS Bedrock conversation from a config entry."""
    agent = BedrockConversationAgent(hass, entry, entry.runtime_data)
    async_add_entities([agent])
    return True


class BedrockConversationAgent(ConversationEntity, AbstractConversationAgent):
